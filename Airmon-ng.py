...
class Feedback:
    ...
    def __init__(self, master):
        ...
        Label(self.frame_content, text = 'Airmon-ng',font=self.headerfont, bg="midnightblue", fg="firebrick").grid(row = 0, column = 0)
        btndetect=Button(self.frame_content, text = 'Detect', fg="cornflowerblue", command =self.canvas_detect, height=2, width=15, font=self.customFont).grid(row = 1, column = 0, padx = 5, pady = 5)
        ...
        #end
        Label(self.frame_content, text = 'Kill these processes',font=self.myfont, bg="midnightblue", fg="deepskyblue").grid(row = 2, column = 1)
        Button(self.frame_content, text = 'Check', fg="cornflowerblue", command =self.canvas_detect_check, height=2, width=15, font=self.customFont).grid(row = 3, column = 0, padx = 5, pady = 5)
        #canvas2=Canvas(self.frame_content, height=200, width=200, bd=2).grid(row = 3, column = 1)
        Button(self.frame_content, text = 'Kill', fg="cornflowerblue", height=2, width=15, font=self.customFont, command=self.killprocess).grid(row = 4, column = 0, padx = 1, pady = 1)
	self.mykill=Text(self.frame_content, height=2, width=15, font=self.customFont)
	self.mykill.grid(row = 4, column = 1, padx = 1, pady = 1)
        btnstart=Button(self.frame_content, text = 'Start', fg="cornflowerblue", command=self.item_selStart, height=2, width=15, font=self.customFont).grid(row = 5, column = 0, padx = 1, pady = 5)
        #btnstart.bind('<ListboxSelect>', self.OnDouble)
        Button(self.frame_content, text = 'Stop', fg="cornflowerblue", height=2, width=15, font=self.customFont, command=self.item_selStop).grid(row = 5, column = 1, padx = 1, pady = 5)
        Label(self.frame_content, text = 'Airvengers',font=self.myfont, bg="midnightblue", fg="firebrick").grid(row = 6, column = 0, columnspan=2)
        self.lilnew1=Listbox(self.frame_content,bg="black", fg="firebrick", font=self.myfont, selectmode=SINGLE)
        self.lilnew1.grid(row = 1, column = 1)
        self.lilnew2=Text(self.frame_content,bg="black", fg="firebrick", font=self.myfont, width=20, height=12)
